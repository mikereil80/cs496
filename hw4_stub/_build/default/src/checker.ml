...
declaration")
  ...
  | DeRef(e) ->
    type_of_expr e >>=
    arg_of_refType "deref: " >>= fun r ->
    return @@ (r)
  | SetRef(e1,e2) ->
    type_of_expr e1 >>=
    arg_of_refType "setref: " >>= fun r1 ->
    type_of_expr e2 >>= fun r2 ->
    if r1=r2
  ...
  | Node(de, le, re) ->
    type_of_expr le >>=
    arg_of_treeType "node: " >>= fun l ->
    type_of_expr re >>=
    arg_of_treeType " " >>= fun r ->
    type_of_expr de >>= fun d ->
    if r=l && r=d
    then return (TreeType(r))
    else error "Must be a node" 
  | NullT(t) ->
    type_of_expr t >>=
    arg_of_treeType "nullt: " >>= fun n ->
    return @@ (BoolType)  
  | GetData(t) ->
    type_of_expr t >>=
    arg_of_treeType "getdata: " >>= fun d ->
    return @@ (d)
  | GetLST(t) ->
    type_of_expr t >>=
    arg_of_treeType "getlst: " >>= fun l ->
    return @@ (TreeType(l)) 
  | GetRST(t) ->
    type_of_expr t >>=
    arg_of_treeType "getrst: " >>= fun r ->
    return @@ (TreeType(r))
