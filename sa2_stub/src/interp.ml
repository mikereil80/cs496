(* Michael Reilly and Julia Martin *)
...
let new_env : string list -> env ea_result  = fun fs ->
     return (List.fold_left (fun acc item ->    
       let l = Store.new_ref g_store (NumVal 0)
       in ExtendEnv(item, (RefVal l), acc)  
       ) EmptyEnv fs)
...
let lookup_method : string -> string -> class_env -> method_decl option = fun c_name m_name c_env ->
  match List.assoc_opt c_name c_env with
  |Some (super,fields,methods) -> List.assoc_opt m_name methods
  |None -> None
...
and
  ...
  | Send(e,m_name,es) ->
    ...
    obj_of_objectVal self >>= fun (c_name,env) ->
    sequence (List.map eval_expr es) >>= fun args ->
